...
graphify v0.5.0+ produces additional visualization formats from the same `graph.json`. **All export flags below are slash-command orchestrator features (run via `/graphify`), not raw CLI flags** — invoking them as Bash (`graphify <path> --svg`) returns `error: unknown command`. Use the slash form:
...
- `/graphify <path> --neo4j-push bolt://localhost:7687` → push directly to a running Neo4j
- graphify HTML is generated by default at `graphify-out/graph.html` on any `/graphify <path>` run

`/kg-canvas` produces Obsidian Canvas (wiki-aware, page-level). Use the slash-form graphify exports above for code-structure-aware views.
